...
PATH_INPUT = '/home/kirian/Downloads/dump_chinese_grammar_wiki_morphSUD_converted'
PATH_OUTPUT = '/home/kirian/Downloads/dump_chinese_grammar_wiki_morphSUD_glossed'


for folder in os.listdir(PATH_INPUT):
    path_folder = os.path.join(PATH_INPUT, folder)
...
#     if file.endswith('.conllu'):
#         path_conll = os.path.join(PATH_CONLL_FOLDERS, file)
#         # path_output = os.path.join(path_folder_output, file)
#         sentences = readConlluFile(path_conll)
#         for sentence in sentences:
#             for token in sentence['treeJson']['nodesJson'].values():
...
#     writeConlluFile(path_conll, sentences, overwrite=True)
